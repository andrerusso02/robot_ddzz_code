...
void diffbot::BaseController<TMotorController, TMotorDriver>::init()
{
    ...
    nh_.loginfo(log_msg.c_str());
    float k_p, k_i, k_d;
    nh_.getParam("/diffbot/pid/k_p", &k_p);
    nh_.getParam("/diffbot/pid/k_i", &k_i);
    nh_.getParam("/diffbot/pid/k_d", &k_d);
    log_msg = String("/diffbot/pid/k_p: ") + String(k_p);
    nh_.loginfo(log_msg.c_str());
    log_msg = String("/diffbot/pid/k_i: ") + String(k_i);
    nh_.loginfo(log_msg.c_str());
    log_msg = String("/diffbot/pid/k_d: ") + String(k_d);
    motor_pid_left_ = PID(PWM_MIN, PWM_MAX, k_p, k_i, k_d);
    motor_pid_right_ = PID(PWM_MIN, PWM_MAX, k_p, k_i, k_d);
    nh_.getParam("/diffbot/pid/pwm_min", &pwm_min_);
    log_msg = String("/diffbot/pid/pwm_min: ") + String(pwm_min_);
...
void diffbot::BaseController<TMotorController, TMotorDriver>::write()
{
    ...
    if(abs(motor_cmd_left_) < pwm_min_ && motor_cmd_left_ != 0)
    {
        motor_cmd_left_ = pwm_min_ * motor_cmd_left_/abs(motor_cmd_left_);
    }
    ...
    msg_motor_cmd_left_.data = motor_cmd_left_/25.5;
    msg_motor_cmd_right_.data = motor_cmd_right_ / 25.5;

    pub_motor_command_left_.publish(&msg_motor_cmd_left_);
    pub_motor_command_right_.publish(&msg_motor_cmd_right_);
}
